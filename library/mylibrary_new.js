...
var RequestTable=(function(){
	return function RequestTable(method,url,param)
	{
		...
		var responseHeaders={};
		var tableHead={};
		var _h=xmlHttpSync.getAllResponseHeaders(),_b=xmlHttpSync.responseText;
		var _testTable=new Table(param.cmd).initWithStrings(_h,_b);
		//console.log(_testTable);
		/*
		var array=xmlHttpSync.getAllResponseHeaders().split("\n");
		//console.log(array);
		for(var index=0;index<array.length;index++){
			...
			if(((h[0]+"").indexOf('Head'))!=-1){
				val=val.split("|");
				tableHead[h[0]]=val;
			}
			responseHeaders[h[0]]=val;
		}
		...
		var headers=tableHead;
		var body=StringToArray(xmlDoc);
		var table={head:tableHead,body:body,columns:{}}
		table.body=body;
		body.splice(0,1)
		for(var c=0;c<tableHead["Table-Head-Names"].length;c++){
			var name=tableHead["Table-Head-Names"][c];
			table.columns[name]=[];
			for(var r=0;r<body.length;r++){
				table.columns[name][r]=body[r][c];
				//table.body[r][c]=body[r][c];;
			}
		}*/
		//console.log(table);
		return _testTable;
...
{
	document.getElementById(selectname).options[document.getElementById(selectname).options.length] = new Option(value,id);
}
...
function AddToSelectFromArray(selectname,array,ignore_records)
{
	...
	{
		AddToSelect(selectname,array[i][0],array[i][1]);
	}
}
