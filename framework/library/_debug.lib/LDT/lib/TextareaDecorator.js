...
function TextareaDecorator( textarea, parser ){
	...
	linenumbers.style.backgroundColor='#AAA';
	linenumbers.style.float='left';
	...
	api.input = textarea;
	api.output = output;
	api.gotoLine=function(line){
		setTimeout(function(){
			//console.log(line,document.getElementById('line_'+line));
			var d=document.getElementById('line_'+line);
			d.style.backgroundColor='#FA9';
			d.scrollIntoView();
		},1);
	}
	api.update = function(){
		...
		if( input ){
			...
			for(var i=0;i<lines;i++){
				...
				d.align='right';d.style.width='100%'
				d.innerHTML=i;
				d.id="line_"+i;
