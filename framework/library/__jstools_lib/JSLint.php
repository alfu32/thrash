...
class JSLEngine {
	...
	public function __construct($binarypath="", $confpath="") {
		// default paths
		$this->base_defines(array(
		"jslint_binary_path" => $this->_fn_fix(dirname(dirname(dirname(__FILE__)))."/3rd/jsl-0.3.0/jsl.exe"),
		"jslint_conf_path" => $this->_fn_fix(dirname(dirname(dirname(__FILE__)))."/3rd/jsl-0.3.0/jsl.default.conf")
		));
	...
	private function _launchLintBinary($input, &$output) {
		...
		$this->fn=$this->_fn_fix(dirname(__FILE__).'/tmp.js');
	...
	public static function _fn_fix($fn,$force="") { // v1.02
		if (strpos($fn,"://") === false) {
	...
	public static function base_defines($x) { // define default
		foreach ($x as $k => $v) {
			if (!defined($k)) {
				define($k,$v);
			}
