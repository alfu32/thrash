...
var THREE=THREE||{};
THREE.Color=function(a){this.autoUpdate=true;this.setHex(a)};
...
THREE.Ray.prototype={
	...
	intersectSolid:function(w,override){
		...
		for(n=0,j=g.faces.length;n<j;n++){
			...
			k=r.dot(v);
			if(k<0 || override){x=v.dot(new THREE.Vector3().sub(t,u))/k;m=u.addSelf(r.multiplyScalar(x));
						if(l(m,t,s,q)||l(m,s,q,p)){o={distance:this.origin.distanceTo(m),point:m,face:i,object:w};e.push(o)}
			}
			}
			return e;
			function l(d,G,D,B){var J=B.clone().subSelf(G),H=D.clone().subSelf(G),E=d.clone().subSelf(G),F=J.dot(J),C=J.dot(H),A=J.dot(E),z=H.dot(H),f=H.dot(E),y=1/(F*z-C*C),K=(z*A-C*f)*y,I=(F*f-C*A)*y;return(K>0)&&(I>0)&&(K+I<1)}
			},
	intersectSolids:function(f,override){var c,a,b,o,n,e=f.objects,d=[];
	for(c=0,a=e.length;c<a;c++){b=e[c];o=b.material[0].opacity;n=(o>.6)||(b.plane)
		if(b instanceof THREE.Mesh || n){d=d.concat(this.intersectObject(b,override));}
		}d.sort(function(h,g){return h.distance-g.distance});return d},
	intersectObject:function(w,override){
		var n,j,i,t,s,q,p,v,k,x,u,r,g=w.geometry,h=g.vertices,o,e=[],m;
		for(n=0,j=g.faces.length;n<j;n++){
			i=g.faces[n];
			u=this.origin.clone();
			...
			if(k<0 || override){x=v.dot(new THREE.Vector3().sub(t,u))/k;m=u.addSelf(r.multiplyScalar(x));
				if(i instanceof THREE.Face3){
					if(l(m,t,s,q)){o={distance:this.origin.distanceTo(m),point:m,face:i,object:w};e.push(o)}
				}else{
					if(i instanceof THREE.Face4){
						if(l(m,t,s,p)||l(m,s,q,p)){o={distance:this.origin.distanceTo(m),point:m,face:i,object:w};e.push(o)
						}
					}
				}
			}
...
THREE.DirectionalLight=function(b,a){THREE.Light.call(this,b);this.position=new THREE.Vector3(0,1,0);this.intensity=a||1};
THREE.DirectionalLight.prototype=new THREE.Light();
THREE.DirectionalLight.prototype.constructor=THREE.DirectionalLight;
THREE.PointLight=function(b,a){THREE.Light.call(this,b);this.position=new THREE.Vector3(0,0,0);this.intensity=a||1};
THREE.DirectionalLight.prototype=new THREE.Light();
THREE.DirectionalLight.prototype.constructor=THREE.PointLight;
THREE.Object3D=function(a){this.position=new THREE.Vector3();this.rotation=new THREE.Vector3();this.scale=new THREE.Vector3(1,1,1);this.matrix=new THREE.Matrix4();this.matrixTranslation=new THREE.Matrix4();this.matrixRotation=new THREE.Matrix4();this.matrixScale=new THREE.Matrix4();this.screen=new THREE.Vector3();this.autoUpdateMatrix=true;this.updateMatrix=function(){this.matrixPosition=THREE.Matrix4.translationMatrix(this.position.x,this.position.y,this.position.z);this.matrixRotation=THREE.Matrix4.rotationXMatrix(this.rotation.x);this.matrixRotation.multiplySelf(THREE.Matrix4.rotationYMatrix(this.rotation.y));this.matrixRotation.multiplySelf(THREE.Matrix4.rotationZMatrix(this.rotation.z));this.matrixScale=THREE.Matrix4.scaleMatrix(this.scale.x,this.scale.y,this.scale.z);this.matrix.copy(this.matrixPosition);this.matrix.multiplySelf(this.matrixRotation);this.matrix.multiplySelf(this.matrixScale)}};

THREE.Particle=function(a){THREE.Object3D.call(this);this.material=a instanceof Array?a:[a];this.autoUpdateMatrix=false};
THREE.Particle.prototype=new THREE.Object3D();
THREE.Particle.prototype.constructor=THREE.Particle;

THREE.Line=function(b,a){THREE.Object3D.call(this);this.geometry=b;this.material=a instanceof Array?a:[a]};

THREE.Line.prototype=new THREE.Object3D();THREE.Line.prototype.constructor=THREE.Line;THREE.Mesh=function(b,a,c){THREE.Object3D.call(this);this.geometry=b;this.material=a instanceof Array?a:[a];this.flipSided=false;this.doubleSided=false;this.overdraw=false;this.materialFaceGroup={};this.sortFacesByMaterial();if(c){this.normalizeUVs()}this.geometry.computeBoundingBox()};

THREE.Mesh.prototype=new THREE.Object3D();
THREE.Mesh.prototype.constructor=THREE.Mesh;
THREE.Mesh.prototype.sortFacesByMaterial=function(){var c,b,e,m,k,h,j,n,d,g={};function a(f){var i=[];for(c=0,b=f.length;c<b;c++){if(f[c]==undefined){i.push("undefined")}else{i.push(f[c].toString())}}return i.join("_")}for(e=0,m=this.geometry.faces.length;e<m;e++){k=this.geometry.faces[e];h=k.material;n=a(h);if(g[n]==undefined){g[n]={hash:n,counter:0}}d=g[n].hash+"_"+g[n].counter;if(this.materialFaceGroup[d]==undefined){this.materialFaceGroup[d]={faces:[],material:h,vertices:0}}j=k instanceof THREE.Face3?3:4;if(this.materialFaceGroup[d].vertices+j>65535){g[n].counter+=1;d=g[n].hash+"_"+g[n].counter;if(this.materialFaceGroup[d]==undefined){this.materialFaceGroup[d]={faces:[],material:h,vertices:0}}}this.materialFaceGroup[d].faces.push(e);this.materialFaceGroup[d].vertices+=j}};
...
THREE.LineColorMaterial=function(c,b,a){this.lineWidth=a||1;this.color=new THREE.Color((b!==undefined?b:1)*255<<24^c)};
...
THREE.MeshPhongMaterial=function(d,a,e,c,b){this.ambient=new THREE.Color((b!==undefined?b:1)*255<<24^d);this.diffuse=new THREE.Color((b!==undefined?b:1)*255<<24^a);this.specular=new THREE.Color((b!==undefined?b:1)*255<<24^e);this.shininess=c;this.opacity=b;this.toString=function(){return"THREE.MeshPhongMaterial ( <br/>ambient: "+this.ambient+", <br/>diffuse: "+this.diffuse+", <br/>specular: "+this.specular+", <br/>shininess: "+this.shininess+", <br/>opacity: "+this.opacity+")"}};

THREE.MeshBitmapMaterial=function(a,b){this.id=THREE.MeshBitmapMaterialCounter.value++;this.bitmap=a;this.mode=b||THREE.MeshBitmapMaterialMode.UVMAPPING;this.toString=function(){return"THREE.MeshBitmapMaterial ( bitmap: "+this.bitmap+", mode: "+this.mode+", id: "+this.id+" )"}};
THREE.MeshBitmapMaterialCounter={value:0};
THREE.MeshBitmapMaterialMode={UVMAPPING:0};

THREE.MeshColorFillMaterial=function(b,a){this.color=new THREE.Color((a!==undefined?a:1)*255<<24^b);this.blending='source-over';this.opacity=a;this.toString=function(){return"THREE.MeshColorFillMaterial ( color: "+this.color+" , blending: '"+this.blending+"' , opacity: "+this.opacity+" )"}};

THREE.MeshColorStrokeMaterial=function(c,b,a){this.lineWidth=a||1;this.color=new THREE.Color((b!==undefined?b:1)*255<<24^c);this.toString=function(){return"THREE.MeshColorStrokeMaterial ( lineWidth: "+this.lineWidth+", color: "+this.color+" )"}};

THREE.MeshFaceMaterial=function(){this.toString=function(){return"THREE.MeshFaceMaterial"}};
THREE.ParticleBitmapMaterial=function(a){this.bitmap=a;this.offset=new THREE.Vector2();this.toString=function(){return"THREE.ParticleBitmapMaterial ( bitmap: "+this.bitmap+" )"}};

THREE.ParticleCircleMaterial=function(b,a){this.color=new THREE.Color((a!==undefined?a:1)*255<<24^b);this.toString=function(){return"THREE.ParticleCircleMaterial ( color: "+this.color+" )"}};

THREE.Scene=function(){this.objects=[];this.lights=[];this.addObject=function(a){this.objects.push(a)};this.removeObject=function(a){var b=this.objects.indexOf(a);if(b!==-1){this.objects.splice(b,1)}};this.addLight=function(a){this.lights.push(a)};this.removeLight=function(a){var b=this.lights.indexOf(a);if(b!==-1){this.lights.splice(b,1)}};this.toString=function(){return"THREE.Scene ( "+this.objects+" )"}};

THREE.Projector=function(){var e=null,c,p,n=[],b,f,l=[],k,m,i=[],j,h,a=[],g=new THREE.Vector4(),d=new THREE.Matrix4(),o=new THREE.Matrix4();
	this.projectSceneWireframe=function(J,G){
		var materialOverride=new THREE.MeshColorFillMaterial(0x010101,1);
		var F,E,D,K,I,B,r,L,q,z,H,u,C,w,A,y,x,t,s,cp;e=[];p=0,f=0,m=0,h=0;
		cp=G.position
		if(G.autoUpdateMatrix){G.updateMatrix()}
		d.multiply(G.projectionMatrix,G.matrix);r=J.objects;
		for(F=0,E=r.length;F<E;F++){
			L=r[F];q=L.matrix;
			if(L.autoUpdateMatrix){L.updateMatrix()}
			if(L instanceof THREE.Mesh){
				o.multiply(d,q);z=L.geometry.vertices;
				for(D=0,K=z.length;D<K;D++){
					H=z[D]
					u=H.positionScreen;u.copy(H.position);
					o.transform(u);
				}
				w=L.geometry.faces;
				for(I=0,B=w.length;I<B;I++){
					...
					if(A instanceof THREE.Face3){
						y=z[A.a];x=z[A.b];t=z[A.c];
								c=n[p]=n[p]||new THREE.RenderableFace3();c.v1.copy(y.positionScreen);c.v2.copy(x.positionScreen);c.v3.copy(t.positionScreen);c.normalWorld.copy(A.normal);L.matrixRotation.transform(c.normalWorld);c.centroidWorld.copy(A.centroid);q.transform(c.centroidWorld);
								c.centroidScreen.copy(c.centroidWorld);
								d.transform(c.centroidScreen);
								c.z=c.centroidScreen.z*1000-990;
								c.meshMaterial=L.material;c.faceMaterial=A.material;
								c.overdraw=L.overdraw;c.uvs=L.geometry.uvs[I];c.color=A.color;e.push(c);p++
					}else{
						if(A instanceof THREE.Face4){
							y=z[A.a];x=z[A.b];t=z[A.c];s=z[A.d];
									b=l[f]=l[f]||new THREE.RenderableFace4();
									b.v1.copy(y.positionScreen);b.v2.copy(x.positionScreen);b.v3.copy(t.positionScreen);b.v4.copy(s.positionScreen);b.normalWorld.copy(A.normal);L.matrixRotation.transform(b.normalWorld);b.centroidWorld.copy(A.centroid);q.transform(b.centroidWorld);
									b.centroidScreen.copy(b.centroidWorld);
									d.transform(b.centroidScreen);
									b.z=b.centroidScreen.z*1000-990;
									/**/b.meshMaterial=L.material;b.faceMaterial=A.material;
									b.overdraw=L.overdraw;b.uvs=L.geometry.uvs[I];b.color=A.color;e.push(b);f++
						}
					}
				}
			}else{
				if(L instanceof THREE.Line){
					o.multiply(d,q);z=L.geometry.vertices;
					for(D=0,K=z.length;D<K;D++){
						H=z[D];u=H.positionScreen;u.copy(H.position);o.transform(u);
						if(D>0){
							C=L.geometry.vertices[D-1];
								var Hm=(H.dc>C.dc)
								//var di=[H,x,t,s];di.sort(function(a,b){return a.d-b.d})///!!!
								var minDist=Hm?C:H,maxDist=Hm?H:C///!!!!
								k=i[m]=i[m]||new THREE.RenderableLine();
								k.v1.copy(H.positionScreen);
								k.v2.copy(C.positionScreen);
								k.z=Math.max(H.positionScreen.z,C.positionScreen.z)*1000-990;
								k.material=L.material;
								k._dist=(minDist.dc+maxDist.dc)*.5//THREE.Vector.dist3(c.centroidWorld,G.position)
								k._near=minDist.dc;k._far=maxDist.dc///!!!
								e.push(k);m++
						}
					}
				}else{
					if(L instanceof THREE.Particle){
						g.set(L.position.x,L.position.y,L.position.z,1);G.matrix.transform(g);G.projectionMatrix.transform(g);L.screen.set(g.x/g.w,g.y/g.w,g.z/g.w);
							j=a[h]=a[h]||new THREE.RenderableParticle();j.x=L.screen.x;j.y=L.screen.y;j.z=L.screen.z;j.rotation=L.rotation.z;j.scale.x=L.scale.x*Math.abs(g.x/g.w-(g.x+G.projectionMatrix.n11)/(g.w+G.projectionMatrix.n14));j.scale.y=L.scale.y*Math.abs(g.y/g.w-(g.y+G.projectionMatrix.n22)/(g.w+G.projectionMatrix.n24));j.material=L.material;j.color=L.color;
							e.push(j);h++
					}
				}
			}
	...
	this.projectScene=function(J,G){
		...
		for(F=0,E=r.length;F<E;F++){
			...
			if(L instanceof THREE.Mesh){
				...
				for(D=0,K=z.length;D<K;D++){
					H=z[D]
					u=H.positionScreen;u.copy(H.position);
					o.transform(u);H.__visible=u.z>0&&u.z<1
				}
				w=L.geometry.faces;
				for(I=0,B=w.length;I<B;I++){
					A=w[I];
					if(A instanceof THREE.Face3){
						...
						if(y.__visible&&x.__visible&&t.__visible){
							if((L.doubleSided||(L.flipSided!=(t.positionScreen.x-y.positionScreen.x)*(x.positionScreen.y-y.positionScreen.y)-(t.positionScreen.y-y.positionScreen.y)*(x.positionScreen.x-y.positionScreen.x)<0))){
								...
								d.transform(c.centroidScreen);
								c.z=c.centroidScreen.z*1000-990;
								//c.z=c.centroidWorld.y;
								c.meshMaterial=L.material;c.faceMaterial=A.material;
								/*if(ovr=='override'){b.meshMaterial=materialOverride;b.faceMaterial=materialOverride}*/c.overdraw=L.overdraw;c.uvs=L.geometry.uvs[I];c.color=A.color;e.push(c);p++
							}
						}
					}else{
						if(A instanceof THREE.Face4){
							...
							if(y.__visible&&x.__visible&&t.__visible&&s.__visible){
								if((L.doubleSided||(L.flipSided!=((s.positionScreen.x-y.positionScreen.x)*(x.positionScreen.y-y.positionScreen.y)-(s.positionScreen.y-y.positionScreen.y)*(x.positionScreen.x-y.positionScreen.x)<0||(x.positionScreen.x-t.positionScreen.x)*(s.positionScreen.y-t.positionScreen.y)-(x.positionScreen.y-t.positionScreen.y)*(s.positionScreen.x-t.positionScreen.x)<0)))){
									...
									d.transform(b.centroidScreen);
									b.z=b.centroidScreen.z*1000-990;
									//b.z=b.centroidWorld.y;
									/**/b.meshMaterial=L.material;b.faceMaterial=A.material;
								/*if(ovr=='override'){b.meshMaterial=materialOverride;b.faceMaterial=materialOverride}*/;b.overdraw=L.overdraw;b.uvs=L.geometry.uvs[I];b.color=A.color;e.push(b);f++
								}
							}
						} else {
							if(A instanceof THREE.FaceN){
		...
		e.sort(function(M,v){return v.z-M.z});
		return e
	};
	this.unprojectVector=function(q,s){var r=new THREE.Matrix4();r.multiply(THREE.Matrix4.makeInvert(s.matrix),THREE.Matrix4.makeInvert(s.projectionMatrix));r.transform(q);return q}
};

THREE.CanvasRenderer=function(){var ovr,black='rgba(0,0,0,.2)';var A=null,v=new THREE.Projector(),s=document.createElement("canvas"),n,r,k,p,t=s.getContext("2d"),D=new THREE.Rectangle(),q=new THREE.Rectangle(),z=new THREE.Rectangle(),l=false,o=new THREE.Color(4294967295),x=new THREE.Color(4294967295),f=new THREE.Color(4294967295),y=Math.PI*2,j=new THREE.Vector2(),h=new THREE.Vector3(),G=new THREE.UV(),F=new THREE.UV(),E=new THREE.UV(),C=new THREE.UV(),d=new THREE.Vector2(),b=new THREE.Vector2();this.domElement=s;this.autoClear=true;this.setSize=function(I,H){n=I;r=H;k=n/2;p=r/2;s.width=n;s.height=r;D.set(-k,-p,k,p)};this.clear=function(){if(!q.isEmpty()){q.inflate(1);q.minSelf(D);t.setTransform(1,0,0,-1,k,p);t.clearRect(q.getX(),q.getY(),q.getWidth(),q.getHeight());q.empty()}};
var lc=new THREE.Color(0x121212);lc.a=1;lc.updateStyleString();lc=lc.__styleString;
var def=3;
function DF(a,b,c,d){return {x:(a-c)/def,y:(b-d)/def}}
//function DF(a,b,c,d){var defx=(a-c)/def,defy=(b-d)/def;return {x:(a-c)/defx,y:(b-d)/defy}}

this.render=function(ac,Z,override){ovr=override?override:false;var ab,K,M,U,aa,Q,N,T,R,O,X,V,J,H,S,P,Y,W,L,I;if(this.autoClear){this.clear()}A=v.projectScene(ac,Z);
	t.clearRect(0,0,t.canvas.width,t.canvas.height);t.setTransform(1,0,0,-1,k,p);l=ac.lights.length>0;if(l){B(ac,f)}for(ab=0,K=A.length;ab<K;ab++){M=A[ab];if(M instanceof THREE.RenderableParticle){R=M.x*k;O=M.y*p;for(U=0,aa=M.material.length;U<aa;U++){T=M.material[U];m(R,O,M,T,ac)}}else{if(M instanceof THREE.RenderableLine){R=M.v1.x*k;O=M.v1.y*p;X=M.v2.x*k;V=M.v2.y*p;t.beginPath();t.moveTo(R,O);t.lineTo(X,V);t.closePath();for(U=0,aa=M.material.length;U<aa;U++){T=M.material[U];e(R,O,X,V,M,T,ac)}}else{if(M instanceof THREE.RenderableFace3){M.v1.x*=k;M.v1.y*=p;M.v2.x*=k;M.v2.y*=p;M.v3.x*=k;M.v3.y*=p;if(M.overdraw){c(M.v1,M.v2);c(M.v2,M.v3);c(M.v3,M.v1)}R=M.v1.x;O=M.v1.y;X=M.v2.x;V=M.v2.y;J=M.v3.x;H=M.v3.y;U=0;aa=M.meshMaterial.length;while(U<aa){T=M.meshMaterial[U++];if(T instanceof THREE.MeshFaceMaterial){Q=0;N=M.faceMaterial.length;while(Q<N){T=M.faceMaterial[Q++];i(R,O,X,V,J,H,M,T,ac)}continue}i(R,O,X,V,J,H,M,T,ac)}}else{
	if(M instanceof THREE.RenderableFace4){
		M.v1.x*=k;M.v1.y*=p;M.v2.x*=k;M.v2.y*=p;M.v3.x*=k;M.v3.y*=p;M.v4.x*=k;M.v4.y*=p;d.copy(M.v2);b.copy(M.v4);
		if(M.overdraw){c(M.v1,M.v2);c(M.v2,M.v4);c(M.v4,M.v1)}
		R=M.v1.x;O=M.v1.y;X=M.v2.x;V=M.v2.y;S=M.v4.x;P=M.v4.y;
		if(M.overdraw){c(M.v3,d);c(M.v3,b)}
		J=M.v3.x;H=M.v3.y;Y=d.x;W=d.y;L=b.x;I=b.y;
		U=0;aa=M.meshMaterial.length;
		while(U<aa){
			T=M.meshMaterial[U++];
			if(T instanceof THREE.MeshFaceMaterial){
				Q=0;N=M.faceMaterial.length;
					while(Q<N){T=M.faceMaterial[Q++];g(R,O,X,V,J,H,S,P,Y,W,L,I,M,T,ac)}continue}
...
function Scanline(y){
	var i,im,_o
	this.py=y;
	this.projectScene=function(_scene,_camera){
		for(i=0,im=_scene.objects.length;i<im;i++){
			_o=_scene.objects[i];
			
		}
	}
}
this.scanlineRender=function(ac,Z,override){
	ovr=override?override:false;
	var ab,K,M,U,aa,Q,N,T,R,O,X,V,J,H,S,P,Y,W,L,I;
	A=v.projectSceneWireframe(ac,Z);
	t.clearRect(0,0,t.canvas.width,t.canvas.height)
	t.setTransform(1,0,0,-1,k,p);
	for(ab=0,K=this.domElement.height;ab<K;ab++){
		M=new Scanline((ab/K-.5)*2)
		U=M.projectScene(ac,Z)
		for(aa=0,Q=U.length;aa<Q;aa++){
			t.strokeStyle=U[aa].material[0].color.__styleString;
			t.beginPath();t.moveTo(U[aa].a.x,U[aa].a.y);t.lineTo(U[aa].b.x,U[aa].b.y);
		}
	}
}
this.renderLine=function(ac,Z,override){
	ovr=override?override:false;
	var ab,K,M,U,aa,Q,N,T,R,O,X,V,J,H,S,P,Y,W,L,I;
	A=v.projectSceneWireframe(ac,Z);
	t.clearRect(0,0,t.canvas.width,t.canvas.height)
	...
	for(ab=0,K=A.length;ab<K;ab++){
		M=A[ab];T=M.material?M.material[0]:M.meshMaterial[0];
			if(M instanceof THREE.RenderableParticle){
				R=M.x*k;O=M.y*p;
				m(R,O,M,T,ac)
			}else{
				if(M instanceof THREE.RenderableLine){
					R=M.v1.x*k;O=M.v1.y*p;X=M.v2.x*k;V=M.v2.y*p;
					t.beginPath();t.moveTo(R,O);t.lineTo(X,V);t.closePath();
					e1(R,O,X,V,M,T,ac)
				}else{
					if(M instanceof THREE.RenderableFace3){
						M.v1.x*=k;M.v1.y*=p;M.v2.x*=k;M.v2.y*=p;M.v3.x*=k;M.v3.y*=p;
						R=M.v1.x;O=M.v1.y;X=M.v2.x;V=M.v2.y;J=M.v3.x;H=M.v3.y;
							i1(R,O,X,V,J,H,M,T,ac)
					}else{
						if(M instanceof THREE.RenderableFace4){
							M.v1.x*=k;M.v1.y*=p;M.v2.x*=k;M.v2.y*=p;M.v3.x*=k;M.v3.y*=p;M.v4.x*=k;M.v4.y*=p;d.copy(M.v2);b.copy(M.v4);
							R=M.v1.x;O=M.v1.y;X=M.v2.x;V=M.v2.y;S=M.v4.x;P=M.v4.y;
							J=M.v3.x;H=M.v3.y;Y=d.x;W=d.y;L=b.x;I=b.y;
							g1(R,O,X,V,J,H,S,P,T)
						}
					}
				}
			}
...
function B(L,J){var I,K,H;J.setRGBA(0,0,0,1);for(I=0,K=L.lights.length;I<K;I++){H=L.lights[I];if(H instanceof THREE.AmbientLight){J.r+=H.color.r;J.g+=H.color.g;J.b+=H.color.b}}}

function u(M,K,J){var I,L,H;for(I=0,L=M.lights.length;I<L;I++){H=M.lights[I];if(H instanceof THREE.DirectionalLight){J.r+=H.color.r;J.g+=H.color.g;J.b+=H.color.b}else{if(H instanceof THREE.PointLight){J.r+=H.color.r;J.g+=H.color.g;J.b+=H.color.b}}}}

function a(N,L,J){var I,M,H,K;for(I=0,M=N.lights.length;I<M;I++){H=N.lights[I];if(H instanceof THREE.DirectionalLight){K=L.normalWorld.dot(H.position)*H.intensity;if(K>0){J.r+=H.color.r*K;J.g+=H.color.g*K;J.b+=H.color.b*K}}else{if(H instanceof THREE.PointLight){h.sub(H.position,L.centroidWorld);h.normalize();K=L.normalWorld.dot(h)*H.intensity;if(K>0){J.r+=H.color.r*K;J.g+=H.color.g*K;J.b+=H.color.b*K}}}}}

function m(J,I,M,P,O){var H,U,S,R,N,L,Q,T,K;
	if(P instanceof THREE.ParticleCircleMaterial){
		if(l){x.copyRGB(f);u(O,M,x);o.copyRGBA(P.color);o.multiplySelfRGB(x);o.updateStyleString()}
			else{o=P.color}
		H=M.scale.x*k;U=M.scale.y*p;z.set(J-H,I-U,J+H,I+U);
		if(!D.instersects(z)){return}
		t.save();t.translate(J,I);t.rotate(-M.rotation);t.scale(H,U);t.beginPath();t.arc(0,0,1,0,y,true);t.closePath();t.fillStyle=!ovr?o.__styleString:ovr;setBlending(t,P.blending);t.fill();setBlending(t);t.restore()}else{if(P instanceof THREE.ParticleBitmapMaterial){Q=P.bitmap;T=Q.width/2;K=Q.height/2;S=M.scale.x*k;R=M.scale.y*p;H=S*T;U=R*K;N=P.offset.x*S;L=P.offset.y*R;z.set(J+N-H,I+L-U,J+N+H,I+L+U);if(!D.instersects(z)){return}t.save();t.translate(J,I);t.rotate(-M.rotation);t.scale(S,-R);t.translate(-T+P.offset.x,-K-P.offset.y);setBlending(t,P.blending);t.drawImage(Q,0,0);setBlending(t);t.restore()}}}

function e(H,N,J,I,K,L,M){
	if(L instanceof THREE.LineColorMaterial){
		if(l){x.copyRGB(f);u(M,K,x);o.copyRGBA(L.color);o.multiplySelfRGB(x);o.updateStyleString()}
		else{o=L.color}
		t.lineWidth=L.lineWidth;t.lineJoin="round";t.lineCap="round";t.strokeStyle=o.__styleString;setBlending(t,L.gCO);t.stroke();setBlending(t);z.inflate(t.lineWidth)}}
		
function e1(H,N,J,I,K,L,M){
	var ss
	if(L instanceof THREE.MeshBitmapMaterial)ss=lc
	else ss=L.color.__styleString
	t.lineWidth=L.lineWidth;t.strokeStyle=ss;t.stroke();}

function i(J,I,H,S,P,O,L,N,M){
	var Q,R,K;
	if(N instanceof THREE.MeshColorFillMaterial){
		if(l){x.copyRGB(f);a(M,L,x);o.copyRGBA(N.color);o.multiplySelfRGB(x);o.updateStyleString()}
		else{o=N.color}
		t.beginPath();t.moveTo(J,I);t.lineTo(H,S);t.lineTo(P,O);t.lineTo(J,I);t.closePath();t.fillStyle=!ovr?o.__styleString:ovr;var tm=t.strokeStyle;t.strokeStyle=lc;setBlending(t,N.blending);t.fill();t.stroke();setBlending(t);t.strokeStyle=tm;}
	else{
		if(N instanceof THREE.MeshColorStrokeMaterial){
			if(l){x.copyRGB(f);a(M,L,x);o.copyRGBA(N.color);o.multiplySelfRGB(x);o.updateStyleString()}
			else{o=N.color}
			t.beginPath();t.moveTo(J,I);t.lineTo(H,S);t.lineTo(P,O);t.lineTo(J,I);t.closePath();t.lineWidth=N.lineWidth;t.lineJoin="round";t.lineCap="round";t.strokeStyle=o.__styleString;setBlending(t,N.blending);t.stroke();setBlending(t);z.inflate(t.lineWidth)}
		else{
			if(N instanceof THREE.MeshBitmapMaterial){Q=N.bitmap;R=Q.width-1;K=Q.height-1;G.copy(L.uvs[0]);F.copy(L.uvs[1]);E.copy(L.uvs[2]);G.u*=R;G.v*=K;F.u*=R;F.v*=K;E.u*=R;E.v*=K;w(Q,J,I,H,S,P,O,G.u,G.v,F.u,F.v,E.u,E.v,N)}}
		}
	}
function i1(J,I,H,S,P,O,L,N,M){
	var Q,R,K,ss;
	if(N instanceof THREE.MeshBitmapMaterial){
		ss=lc
		}
	else{
		ss=N.color.__styleString
		}
		t.beginPath();t.moveTo(J,I);t.lineTo(H,S);t.lineTo(P,O);t.lineTo(J,I);t.closePath();t.lineWidth=.5;t.strokeStyle=ss;t.stroke()}

function g(N,M,V,T,I,H,P,O,W,U,K,J,L,R,X){
var Y,Q,S;
	//var a1=DF(N,M,V,T),b1=DF(V,T,I,H),c1=DF(I,H,P,O),d1=DF(P,O,N,M)

	if(R instanceof THREE.MeshColorFillMaterial){
		if(l){x.copyRGB(f);a(X,L,x);o.copyRGBA(R.color);o.multiplySelfRGB(x);o.updateStyleString()}
		else{o=R.color}
		/*markText(t,'Mesh2',N,M)*/;t.beginPath();t.moveTo(N,M);t.lineTo(V,T);t.lineTo(I,H);t.lineTo(P,O);t.lineTo(N,M);t.closePath();t.fillStyle=!ovr?o.__styleString:ovr;var tm=t.strokeStyle;t.strokeStyle=lc;setBlending(t,R.blending);t.fill();t.stroke();setBlending(t);t.strokeStyle=tm}
	else{
		if(R instanceof THREE.MeshColorStrokeMaterial){
			if(l){x.copyRGB(f);a(X,L,x);o.copyRGBA(R.color);o.multiplySelfRGB(x);o.updateStyleString()}
			else{o=R.color}
			t.beginPath();t.moveTo(N,M);t.lineTo(V,T);t.lineTo(I,H);t.lineTo(P,O);t.lineTo(N,M);t.closePath();t.lineWidth=R.lineWidth;t.lineJoin="round";t.lineCap="round";t.strokeStyle=o.__styleString;setBlending(t,R.blending);t.stroke();setBlending(t);z.inflate(t.lineWidth)}
		else{
			if(R instanceof THREE.MeshBitmapMaterial){gO=R;Y=R.bitmap;Q=Y.width-1;S=Y.height-1;G.copy(L.uvs[0]);F.copy(L.uvs[1]);E.copy(L.uvs[2]);C.copy(L.uvs[3]);G.u*=Q;G.v*=S;F.u*=Q;F.v*=S;E.u*=Q;E.v*=S;C.u*=Q;C.v*=S;w(Y,N,M,V,T,P,O,G.u,G.v,F.u,F.v,C.u,C.v,R);w(Y,W,U,I,H,K,J,F.u,F.v,E.u,E.v,C.u,C.v,R)}
		}
	}
		t.lineWidth=.3;
		t.strokeStyle=lc;
		//for(var iz=0;iz<def+1;iz++){
		//t.beginPath();t.moveTo(N-a1.x*iz,M-a1.y*iz);t.lineTo(P+c1.x*iz,O+c1.y*iz);t.closePath();t.stroke()}
		//for(var iz=0;iz<def+1;iz++){
		//t.beginPath();t.moveTo(V-b1.x*iz,T-b1.y*iz);t.lineTo(I+d1.x*iz,h+d1.y*iz);t.closePath();t.stroke()}
		t.beginPath();t.moveTo(N,M);t.lineTo(V,T);t.lineTo(I,H);t.lineTo(P,O);t.lineTo(N,M);t.closePath();t.stroke()
}
function g1(N,M,V,T,I,H,P,O,R){
	var ss;
	var a1=DF(N,M,V,T),b1=DF(V,T,I,H),c1=DF(I,H,P,O),d1=DF(P,O,N,M)
		if(R instanceof THREE.MeshBitmapMaterial){ss=lc}
		else{ss=R.color.__styleString}
		t.lineWidth=.3;
		t.strokeStyle=ss;
		//for(var iz=0;iz<def+1;iz++){
		//t.beginPath();t.moveTo(N-a1.x*iz,M-a1.y*iz);t.lineTo(P+c1.x*iz,O+c1.y*iz);t.closePath();t.stroke()}
		...
		//t.beginPath();t.moveTo(V-b1.x*iz,T-b1.y*iz);t.lineTo(I+d1.x*iz,h+d1.y*iz);t.closePath();t.stroke()}
	//t.lineWidth=1;
	t.beginPath();t.moveTo(N,M);t.lineTo(V,T);t.lineTo(I,H);t.lineTo(P,O);t.lineTo(N,M);t.closePath();t.lineWidth=.5;t.strokeStyle=ss;t.stroke()
}

function w(aa,P,O,V,U,J,H,T,S,X,W,L,K,gO){var I,Z,Y,N,M,R,Q;t.beginPath();t.moveTo(P,O);t.lineTo(V,U);t.lineTo(J,H);t.lineTo(P,O);t.closePath();t.save();t.clip();I=T*(K-W)-X*K+L*W+(X-L)*S;Z=-(S*(J-V)-W*J+K*V+(W-K)*P)/I;Y=(W*H+S*(U-H)-K*U+(K-W)*O)/I;N=(T*(J-V)-X*J+L*V+(X-L)*P)/I;M=-(X*H+T*(U-H)-L*U+(L-X)*O)/I;R=(T*(K*V-W*J)+S*(X*J-L*V)+(L*W-X*K)*P)/I;Q=(T*(K*U-W*H)+S*(X*H-L*U)+(L*W-X*K)*O)/I;t.transform(Z,Y,N,M,R,Q);setBlending(t,gO.blending);/*con.out=[Z,Y,N,M,R,Q]*/;var ptrn=t.createPattern(aa,'repeat');var tmpfl=t.fillStyle; t.fillStyle=!ovr?ptrn:ovr; t.fillRect(-0,-0,4096,4096); t.fillStyle=tmpfl;/*markText(t,gO.blending,0,0)*/; /*t.drawImage(aa,0,0)*/;setBlending(t);t.restore()}
...
setBlending=function(ct,va){if(va){ct.globalCompositeOperation=va}else{ct.globalCompositeOperation='source-over'};}
markText=function(ct,va,x,y){ct.font='20px sans-serif ';var f=ct.fillStyle; ct.translate(x,y);ct.rotate(3.14);ct.scale(-1,1); ct.fillStyle='#3366ff'; ct.fillText(va,0,0); ct.fillStyle=f;ct.scale(-1,1); ct.rotate(-3.14);ct.translate(-x,-y)}